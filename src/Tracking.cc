    ...
    bStepByStep(false)
{
    ...
    if(sensor==System::IMU_MONOCULAR || sensor==System::IMU_STEREO || (mSensor == System::IMU_RGBD))
    {
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames) // MaxFrames = fps
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        std::chrono::steady_clock::time_point timeStartPosePredict = std::chrono::steady_clock::now();
            ...
            if(mState==OK)
            {
                ...
                if (!bOK)
                {
                    ...
                    {
                        cout << "Enough KF in map: " << pCurrentMap->KeyFramesInMap() << endl;
                        cout << "Track with MotionModel and ReferenceKeyFrame failed, System state set to be RECENTLY_LOST!" << endl;
        ...
        if(bOK)
            mState = OK;
        else if (mState == OK) // Current Frame track lost and Current Frame's state is OK, No Possible
        {
            if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
            {
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    // if use IMU and IMU has obtained good initial values for the inertial variables
    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || (mSensor == System::IMU_RGBD)) &&
...
bool Tracking::TrackWithMotionModel()
{
    ...
    {
        // Predict current frame's state with IMU if it is initialized and it doesnt need reset
        PredictStateIMU();
        return true;
    }
    else
...
bool Tracking::TrackLocalMap()
{
    ...
    // TODO check outliers before PoseOptimization
    int aux1 = 0, aux2=0;
    for(int i=0; i<mCurrentFrame.N; i++)
        if( mCurrentFrame.mvpMapPoints[i])
        {
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    // Do not insert keyframes if not enough frames have passed since last relocalisation
    // MaxFrames = FramesToResetIMU = fps
    ...
    // Condition 1a: More than "MaxFrames" have passed from last keyframe insertion, MaxFrames=FramesToResetIMU=fps
    const bool c1a = mCurrentFrame.mnId>=mnLastKeyFrameId+mMaxFrames;
    // Condition 1b: More than "MinFrames" have passed and Local Mapping is idle, mMinFrames=0
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    // TODO check the condition on using current or last frame
...
void Tracking::UpdateFrameIMU(const float s, const IMU::Bias &b, KeyFrame* pCurrentKeyFrame)
{
    ...
    for(list<cv::Mat>::iterator lit=mlRelativeFramePoses.begin(),lend=mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lbL++)
    {
        ...
        if(pKF->GetMap() == pMap) // update all Frame's translation scale
        {
            (*lit).rowRange(0,3).col(3)=(*lit).rowRange(0,3).col(3)*s;
        }
    }

    // Update Last Frame, Last KeyFrame, Last Bias for tracking
    mLastBias = b;
